...
class ThreeLayerNet:
    def __init__(self, n_input, n_hidden1, n_hidden2, n_output, learning_rate=0.01):
        W1 = 0.01 * np.random.randn(n_input, n_hidden1)
        b1 = np.zeros(n_hidden1)
        W2 = 0.01 * np.random.randn(n_hidden1, n_hidden2)
        b2 = np.zeros(n_hidden2)
        W3 = 0.01 * np.random.randn(n_hidden2, n_output)
    ...
    def loss(self, output, label):
        loss = self.loss_layer.forward(output, label)
